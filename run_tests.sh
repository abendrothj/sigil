...
if [ "$1" == "verbose" ] || [ "$1" == "-v" ]; then
    ...
    pytest tests/ --cov=core --cov=api --cov-report=html --cov-report=term-missing
    echo ""
    ...
    pytest tests/test_crypto_signatures.py -v
elif [ "$1" == "api" ]; then
    echo "Running API tests only..."
    pytest tests/test_api.py -v
elif [ "$1" == "cli" ]; then
    echo "Running CLI tests only..."
    pytest tests/test_secure_seed.py -v
    ...
    echo "  unit        - Run only unit tests (crypto_signatures)"
    echo "  api         - Run only API tests"
    echo "  cli         - Run only CLI tests (secure_seed)"
